...
{
    start -> seed parent -> number of parents -> survival;

    a [as=start];

    { [same layer] seed parent, b [as=female], c [as=hermaphrodite] };
    { [same layer] number of parents, d [as=1], e [as=2]};
    { [same layer] survival, f [as=alive], g [as=dead] };

    a -> {
        b [> "$\frac {\sigma N_f} {\sigma N_f + N_h}$"],
        c [> "$\frac {N_h} {\sigma N_f + N_h}$"]
    },
    b -> ["$i $"] e -> {f, g},
    c -> {d, e},
    d -> {f, g},
...
\graph[layered layout]
{
    start -> number of parents -> seed parent -> survival;

    a [as=start];

    { [same layer] number of parents, b [as=1], c [as=2] },
    { [same layer] seed parent, d [as={biparental,female}], e [as=hermaphrodite] };
    { [same layer] survival, f [as=alive], g [as=dead] };
