...
def get_population(simu, size, loci, info_fields="self_gen"):
    ...
    return simu.Population(size=size, ploidy=2, loci=loci, infoFields=info_fields)
        ...
        def fundamental_generator(pop):
            ...
            while True:
                if runif() < stilde:  # proportion of self-fertilized eggs
                    if (
                        runif() < tau
                    ):  # survival of a uniparental zygote (rel to a biparental z)
                        yield rint(npop)
                else:
                    first, second = rint(npop), rint(npop)
                    while first == second:
        ...
        def compound_generator(pop):
            ...
            while True:
                ...
                if runif() < stilde:  # proportion of self-fertlized egg
                    if (
                        runif() < tau
                    ):  # survival rate of a uniparental zygote rel to a biparental z.
                        yield herms.individual(rint(nherm))
                else:  # biparental
                    yield [males.individual(rint(nmale)), herms.individual(rint(nherm))]
...
def get_pure_hermaphrodite_mating(
    simu, r_rate, parents_chooser, size, rec_sites, field="self_gen"
):
    """
    ...
    return simu.HomoMating(
        chooser=parents_chooser,
        generator=simu.OffspringGenerator(
            ops=[simu.Recombinator(rates=r_rate, loci=rec_sites), selfing_tagger]
        ),
        subPopSize=size,
    )
...
def get_androdioecious_mating(
    simu, r_rate, parents_chooser, size, sex_seq, rec_sites, field="self_gen"
):
    """
...
def pure_hermaphrodite(simu, execute_func, config):
    ...
    """
    pop = get_population(
        simu=simu, size=config.N, loci=config.loci * config.allele_length
    )
    ...
    mating_op = get_pure_hermaphrodite_mating(
        simu,
        r_rate=config.r,
...
def androdioecy(simu, execute_func, config):
    ...
    pop = get_population(simu=simu, size=N, loci=config.loci * config.allele_length)
    sex_seq = tuple(simu.MALE for _ in range(N - Nh)) + tuple(
        simu.FEMALE for _ in range(Nh)
    )
    ...
    parents_chooser = simu.PyParentsChooser(pick_androdioecious_parents(simu, config))

    mating_op = get_androdioecious_mating(
        simu,
        r_rate=config.r,
        parents_chooser=parents_chooser,
        size=config.N,
...
def gynodioecy(simu, execute_func, config):
    ...
    Nh = config.N_hermaphrodites
    pop = get_population(simu=simu, size=N, loci=config.loci * config.allele_length)
    sex_seq = tuple(simu.MALE for _ in range(Nh)) + tuple(
        simu.FEMALE for _ in range(N - Nh)
    )
    ...
    parents_chooser = simu.PyParentsChooser(pick_gynodioecious_parents(simu, config))

    mating_op = get_gynodioecious_mating(
        simu,
        r_rate=config.r,
        parents_chooser=parents_chooser,
        size=config.N,
